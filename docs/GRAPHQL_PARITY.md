...
| 02  | `users`                       | `users/`           | `usersService.ts`                                        | —                      | `/admin` (user list)                                         | ✅ Wired |
...
| 15  | `admin`                       | `admin/`           | `adminService.ts`                                        | `useAdmin.ts`          | `/admin`, `/admin/deployments`                               | ✅ Wired |
...
| adminService        | Fixed `userStats` selection (`totalUsers`, `activeUsers`, `usersByRole`, `usersByPlan`); `logs(LogQueryFilterInput)` + pageInfo; `searchLogs(input: LogSearchInput)`; `userHistory(filters: UserHistoryFilterInput)` + all `UserHistoryItem` fields | ✅ Done |
...
| admin/page.tsx      | Updated stat grid: `usersByRole`/`usersByPlan` JSON; log table uses `timestamp`/`logger`                                                                                                                                                            | ✅ Done |
...
| AdminLogTable       | Uses `log.timestamp` / `log.logger` (was `log.createdAt` / `log.service`)                                                                                                                                                                           | ✅ Done |
| AdminStatGrid       | Uses `usersByRole` / `usersByPlan` (was `usersBySubscription` / `newUsersThisMonth`)                                                                                                                                                                | ✅ Done |
