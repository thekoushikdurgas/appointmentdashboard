...
| admin module (app)  | Customer app has no `/admin` UI; operator console is **`contact360.io/admin`** (Next.js) calling gateway `admin.*`, `billing.*`, `jobs.*`, etc. Legacy Django DocsAI (`contact360.io/1`) runs in parallel until parity matrix is complete. | ✅ Done |
...
| Admin UI components | **Removed from customer app** (`feature/admin/*`); ops use **`contact360.io/admin`** Next console + legacy Django iframe routes                                                                                                            | ✅ Done |
