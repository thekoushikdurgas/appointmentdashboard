...
import { Loader2, UserCircle2, Users, Linkedin, X } from "lucide-react";
import { Button } from "@/components/ui/Button";
import { Badge } from "@/components/ui/Badge";
import { MediaObject } from "@/components/ui/MediaObject";
import { Pagination } from "@/components/ui/Pagination";
import {
  fetchJobCompanyProfile,
  fetchJobCompanyContacts,
  asRecord,
...
type CompanyContactsDrawerState =
  | { kind: "idle" }
  | { kind: "loading" }
...
export interface JobCompanyContactsDrawerProps {
  job: LinkedInJobRow;
  isOpen: boolean;
  onClose: () => void;
...
/**
 * job.server read-through: enriched company profile + contacts for this job.
 */
export function JobCompanyContactsDrawer({
  job,
  isOpen,
  onClose,
  density = "comfortable",
}: JobCompanyContactsDrawerProps) {
  const [state, setState] = useState<CompanyContactsDrawerState>({
    kind: "idle",
  });
    ...
    (async () => {
      ...
      try {
        const [co, deptSample] = await Promise.all([
          fetchJobCompanyProfile(job.linkedinJobId),
          fetchJobCompanyContacts(job.linkedinJobId, {
            page: 1,
        ...
        if (!a.ok) {
          setState({
            kind: "error",
            message: sanitizeUserFacingMessage(a.message),
          });
          return;
        }
        ...
        const msg = sanitizeUserFacingMessage(
          e instanceof Error ? e.message : "Failed to load company data",
        );
        setState({ kind: "error", message: msg });
        toast.error("Company data", { description: msg });
      }
    })();
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const ct = await fetchJobCompanyContacts(job.linkedinJobId, {
          page: contactPage,
        ...
        if (!parsed.ok) {
          toast.error("Company contacts", {
            description: sanitizeUserFacingMessage(parsed.message),
          });
          return;
        }
        setState((prev) =>
        ...
        toast.error("Company contacts", { description: msg });
      } finally {
  ...
  const companyNameMismatch =
    ...
    rowNameNorm !== enrichedCompanyNameNorm;
  ...
  return (
    <HiringSignalAsideDrawer
      isOpen={isOpen}
      onClose={onClose}
      ariaLabelledBy="c360-hs-job-company-contacts-title"
    >
        ...
        <div className="c360-min-w-0 c360-flex-1">
          {state.kind === "ok" && state.company ? (
            <MediaObject
              ...
              title={
                <h2
                  id="c360-hs-job-company-contacts-title"
            ...
            <h2
              ...
              data-tour="hs-contacts-panel-head"
            >
              {job.companyName?.trim() || "Company for this role"}
            </h2>
          )}
          {state.kind === "ok" && !state.company ? (
            <p className="c360-mt-2 c360-text-2xs c360-text-ink-muted">
              No enriched company record yet. Data may appear after the next
              company sync.
            </p>
          ) : null}
        </div>
      ...
      <div className="c360-hs-drawer__body">
        {companyNameMismatch ? (
          <p className="c360-mb-2 c360-text-2xs c360-text-ink-muted">
            Hiring table showed “{job.companyName}”; enriched company name is
            shown above.
          </p>
        ) : null}
        ...
        {state.kind === "error" ? (
          <p
            ...
            role="alert"
          >
            {sanitizeUserFacingMessage(state.message)}
          </p>
        ) : null}
          ...
          <div className="c360-space-y-4">
            {state.poster ? (
              <section
                ...
                aria-labelledby="c360-hs-job-poster"
              >
                <h3
                  id="c360-hs-job-poster"
            ...
            <section
              ...
              aria-labelledby="c360-hs-company-people"
            >
              <div className="c360-hs-drawer__section-toolbar c360-flex c360-flex-wrap c360-items-center c360-gap-2">
                <h3
                  id="c360-hs-company-people"
