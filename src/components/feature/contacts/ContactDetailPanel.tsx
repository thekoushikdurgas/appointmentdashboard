...
interface ContactDetailPanelProps {
  contact: Contact;
  /** Table column count for expanded HTML `<tr>` row (native tables only). */
  colSpan?: number;
  /**
   * `tableRow` — render as `<tr><td colSpan=…>` (legacy contacts `<table>`).
   * `block` — render as a `<div>` for use below MUI DataGrid or other non-table layouts.
   */
...
export function ContactDetailPanel({
  ...
  if (layout === "block") {
    return (
      <div className="c360-contact-detail-expand-cell c360-contact-detail-expand-cell--block">
        {inner}
      </div>
    );
  }
