...
/** Static sidebar enum facets (job.server fixed-list fields). */
export type HireSignalEnumFilterField =
  | "employment_type"
  | "workplace_type"
  | "experience_bucket"
  | "education_level_min"
...
export type HireSignalJobFilterField =
  | "title"
  | "company"
  | "location"
  | "company_funding"
...
export async function fetchHireSignalJobFilterOptions(
  field: HireSignalJobFilterField,
  filters: JobListFilters,
...
export async function fetchJobCompanyProfile(linkedinJobId: string) {
  return graphqlQuery<{
    hireSignal: { jobConnectraCompany: HireSignalApiJson };
  }>(HIRE_SIGNAL_JOB_CONNECTRA_COMPANY, { linkedinJobId }, HS_GQL);
}

export async function fetchJobCompanyContacts(
  linkedinJobId: string,
  options?: {
    page?: number;
    limit?: number;
...
export async function fetchEnrichedCompany(companyUuid: string) {
  return graphqlQuery<{
