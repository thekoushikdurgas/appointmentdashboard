...
/** Drop a cached GraphQL query result (e.g. after a mutation). */
export function invalidateGraphqlQueryCache(
  query: string,
  variables?: Record<string, unknown>,
): void {
  tryLocalStorageRemove(CACHE_PREFIX + buildInflightKey(query, variables));
}
